...
class Solution:
    ...
    def solution_0_level_is_only(self, sudoku_board_original):
        current_sudoku_board1 = self.try_to_decide1(sudoku_board_original)
        start_empty_cells, start_contradictions = self.get_board_info(current_sudoku_board1)
        stop_test = start_contradictions != 0

        while not stop_test:
            current_sudoku_board2 = self.try_to_decide1(current_sudoku_board1)
            cur_empty_cells, cur_contradictions = self.get_board_info(current_sudoku_board2)

            if cur_empty_cells < start_empty_cells and cur_contradictions == 0:
                start_empty_cells = cur_empty_cells
                current_sudoku_board1 = current_sudoku_board2
            else:
                stop_test = True

        result = cur_empty_cells == 0 and cur_contradictions == 0
        return result

    def solution_1_level_is_only(self, sudoku_board_original):
        current_sudoku_board1 = self.try_to_decide1(sudoku_board_original)
        start_empty_cells, start_contradictions = self.get_board_info(current_sudoku_board1)
        stop_test = start_contradictions != 0
